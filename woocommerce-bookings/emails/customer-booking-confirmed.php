...
<p><?php _e( 'We have availability for your requested booking below. Your booking will be confirmed after payment is received', 'woocommerce-bookings' ); ?></p>
...
<table cellspacing="0" cellpadding="6" style="width: 100%; border: 1px solid #eee;" border="1" bordercolor="#eee">
	<tbody>
		...
		<tr>
			<th style="text-align:left; border: 1px solid #eee;" scope="row"><?php _e( 'Booking Date', 'woocommerce-bookings' ); ?></th>
...
?>
	...
	<?php if ( 'pending' === $order->get_status() ) : ?>
		...
		<p>
		<?php
		printf( __( 'Total booking cost (JPY): %s', 'woocommerce-bookings' ), $order->get_total() );
		?>
		</p>
		<p>
		<?php
		/* translators: 1: checkout payment url */
		printf( __( 'To confirm this booking please pay using the following link: %s', 'woocommerce-bookings' ), '<a href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . __( 'PAY FOR BOOKING', 'woocommerce-bookings' ) . '</a>' );
		?>
		</p>
	<?php endif; ?>
